    ...
    async claimRunLease(input) {
        ...
        try {
            ...
            const scopes = Array.isArray(response.scopes)
                ? response.scopes.filter((scope) => typeof scope === "string" && scope.length >= 1 && scope.length <= 160)
                : [];
            const rooms = Array.isArray(response.rooms)
                ? response.rooms.filter((room) => typeof room === "string" && room.length >= 1 && room.length <= 160)
                : [];
            this.secretStore.setRunLease({
                runId,
                runSecret,
                expiresAt,
                leaseHeaders,
                leasePolicyId: input.leasePolicyId,
                scopes,
                rooms,
            });
            return {
