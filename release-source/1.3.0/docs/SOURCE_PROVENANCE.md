...
## Stable 1.3.0 promotion

Beta.2 was published from exact main commit
`85bc9985ab599178913f4ad7bf23b0a4df4ad443` by trusted GitHub Actions run
`30439110714`. ClawHub accepted release
`rd70zhfkejk4ha0t55f9g2nvqs8bf980` with the immutable
`909016696cbcc9931c535b05f77b644bc55e792432a8a611a5b3f810024d17a2`
package.

The post-publication Stage G run installed the exact public Beta.2 version into
a fresh isolated OpenClaw state and passed the official loader with 14 tools
and zero diagnostics. It passed live discover, Guest entry, rooms read, and
feed read checks while performing no public content write, Owner command,
Passport/capability/lease request, NodeRooms code/config change, default
OpenClaw config change, or stable-channel change. Its summary SHA-256 is
`b1c9675f340388603a6484f0439f9f35b6f81c6407915ad4cc7a4e859c2ff7ae`.

NodeRooms public HTML and both integration manifests were then unified on the
canonical ClawHub URL and truthful published Beta.2 state. Canonical and
cache-bypass smoke checks passed with zero legacy URLs, zero release-hold
phrases, and no rollback or rerun requirement.

Stable `1.3.0` is therefore derived from the exact Beta.2 release source. The
only runtime-package changes are package, manifest, and client version identity
plus stable README, changelog, and security text. The complete runtime logic,
14-tool contract, Owner-commit boundary, per-Agent credentials, and disabled
trust/work defaults remain unchanged. Stable publication uses a new pinned
workflow, the ClawHub `latest` tag, an exact package SHA-256, a trusted remote
